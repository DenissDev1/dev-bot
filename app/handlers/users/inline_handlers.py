...
from app.data.convert import ERROR, PREPODS, sticker
from app.data.messages import donuts
...
from app.utils.db_api.commands.coomands_group import select_groups_limit, select_group_id
...
async def get_teachers(inline_query: InlineQuery):
    ...
    if data:
        ...
        not_found = 'Нема такого викладача'
...
async def get_other_schedule(call: CallbackQuery, callback_data: dict):
    ...
    week = ThisNextWeek[str(callback_data.get('this_or_next'))]
    day = Week[str(callback_data.get('day'))]
    week_text = 'наступної' if week == ThisNextWeek.next_week else 'цієї'
    day_txt = day.value.title()
    result_message = [hbold(f"{day_txt} на {week_text} тижні у групи {group.group}")]
    ...
    await call.answer(f"{day_txt} на {week_text} тижні")
...
async def set_rating_teacher(call: CallbackQuery, user: User, callback_data: dict):
    ...
    txt.append(hitalic(f"Ви поставили {rating}\nРейтинг: {rate}/5, кількість оцінок: {teacher.count}"))
    await call.answer('Рейтинг оновлений.')
...
async def delete_teacher_rating_func(call: CallbackQuery, user: User, callback_data: dict):
    ...
    txt = [hbold(f"{teacher.full_name}\n")]
    if teacher.count == 0:
        txt.append(hitalic('Ви відмінили оу.\nПоки що у викладача нема оцінок.'))
    else:
        rate = round(teacher.rating / teacher.count, 1)
        txt.append(hitalic(f"Ви відмінили оцінку.\nРейтинг: {rate}/5, кількість оцінок : {teacher.count}"))
    await call.answer('Рейтинг оновлено.')
...
async def get_teacher_schedule(call: CallbackQuery, callback_data: dict):
    teacher_id = str(callback_data.get('teacher_id'))
    week = ThisNextWeek[str(callback_data.get('this_or_next'))]
    week_text = 'наступній' if week == ThisNextWeek.next_week else 'цій'
    teacher = await select_teacher_id(int(teacher_id))
    teacher_name = teacher.full_name
    text = await select_rows_by_teacher(int(teacher_id), f'{teacher_name} на {week_text} тижні', week)
    await call.answer()
...
async def reset_user(call: CallbackQuery):
    await call.answer('Налаштування скинуто.')
    await States.GROUP.set()
    await call.message.answer('Знайти свою групу:', reply_markup=search_kb)
