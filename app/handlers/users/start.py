...
async def bot_start(message: types.Message):
    ...
    await message.answer(f"Привіт, {message.from_user.full_name}!\n"
                         "Знайди свою группу:", reply_markup=search_kb)
...
async def check_group(message: types.Message, state: FSMContext):
    ...
    await message.answer('Виберіть свою підгрупу:', reply_markup=subgroup_menu(group.subgroups))


@dp.message_handler(state=States.GROUP)
async def failed_process_group(message: types.Message):
    await message.answer('Щось не так. Знайдіть свою групу:', reply_markup=search_kb)
...
async def failed_process_subgroup(message: types.Message):
    await message.answer('Виберіть свою підгрупу☝️')
